...
def call_search(state: ContextState):
    """Perform a similarity search in Pinecone for each namespace."""
    ...
    for ns in state["namespaces"]:
        try:
            ...
            for doc, score in batch:
                if score >= 0.6:
                    accum.append(doc)
            print("Found", len(accum), "results")
        except Exception as e:
            print("Error during Pinecone search:", str(e))
            # Handle error by setting an error message in the state
            state["error"] = str(e)
            return state
        
    print("Accumulated results:", len(accum))
...
def build_and_run_react_agent(state: GenerateState):
    print("Building and running REACT agent...")
    llm = state["chat"]
    system =  SystemMessage(content="""
            Use a Tool only when you truly need it to answer the user.
            If no Tool is needed, output nothing (an empty string).
            Return only the Tool’s result—or nothing—without extra text.
            """)
    print("System message:", system.content)
    agent = create_react_agent(llm, tools=Tools, prompt=system)
    print("Agent created.")

    msgs = list(state.get("chat_history", []))
    print("Chat history:", msgs)

    msgs.append(HumanMessage(content=state["prompt"]))
    print("User message:", msgs[-1].content)
...
def merge_branches(state: GenerateState):
    # both branches have already written into state:
    print("Agent response:", state["response"])
    print("Context:", state["context"])
    return state

def generate_combined_response(state: GenerateState):
    print("Generating combined response...")
    ...
    sys_msg = SystemMessage(content=f"""
        Combine the “Agent answer” and the “Context” below into **one** clear, complete reply for the user.

        • Keep it brief and accurate.  
        • Remove any repetition or fluff.  
    ...
    print("Agent message:", state["response"])
    print("Context:", state["context"])
    hum_msg = HumanMessage(content=state["prompt"])

    try:
        print("Sending to LLM...")
        final = llm.invoke([sys_msg, hum_msg])
        state["response"] = final.content
        print("Final response:", state["response"])
        
        state['chat_history'].append(["user", hum_msg.content])
        state['chat_history'].append(["ai", final.content])
    except Exception as e:
        print("Error during LLM invocation:", str(e))
        state["error"] = str(e)
    return state

def handle_error(state: GenerateState):
    """Handle errors in the state graph."""
    print("Handling error...")
    print("Error message:", state.get("error", ""))
...
builder.add_node("build_and_run_react_agent",  build_and_run_react_agent)
...
# 2) if session ok → get context, else error
builder.add_edge(
    "create_live_session","get_relevant_context"
)

# 3) if context ok → run agent, else error
builder.add_edge(
    "get_relevant_context", "build_and_run_react_agent"
)

# 4) if agent ok → final combine, else error
builder.add_edge(
    "build_and_run_react_agent","generate_combined_response"
)
# 4) if agent ok → final combine, else error
# builder.add_edge(
#     "get_relevant_context","generate_combined_response"
# )

# 5) if final ok → end, else error
builder.add_edge(
    "generate_combined_response", END
)


generate_graph_3 = builder.compile()